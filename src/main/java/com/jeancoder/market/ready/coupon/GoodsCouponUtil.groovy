...
class GoodsCouponUtil extends CouponUtilAbstract {

	private static final JCLogger LOGGER = LoggerSource.getLogger(GoodsCouponUtil.class.getName());
	...
	public def getCouponDto(def batch, def couponCode, def rule) {
		...
		if (CouponConstants._coupon_type_exchange_.equals(batch.coupon_type)) {
			...
			if (values.length == 3) {
				rules.add(values[0] + "张卡劵兑换" + values[1] + "个商品补"+ MoneyUtil.get_yuan_from_cent(values[2]) + "元")
			}
...
//		CouponBatch cb = CouponBatchService.INSTANSE.getById(couponCode.batch_id);
//		GoodsCouponRule goodsCouponRule = GoodsCouponRuleServer.INSTANSE.getRule_ByBathId(couponCode.batch_id);
//		//判断是否有可用的门店
//		boolean flags = true;
//		String[] gocs = goodsCouponRule.stores.split(",");
//		for (goc in gocs) {
...
//		}
//		if(flags){
//			return AvailabilityStatus.notAvailable("没找到可用的门店");
//		}
//		//判断是否有可用的商品
//		flags = true;
//		String[] gocs1 = goodsCouponRule.goods.split(",");
//		for (goc in gocs1) {
//			for(good in goods){
...
//		}
//		if(flags){
//			return AvailabilityStatus.notAvailable("没找到可用的商品");
//		}
//
//		//判断状态能不能用和有没有失效
//		for (ccd in couponCodes) {
//			if(!ccd.status.equals(CouponConstants._coupon_code_status_touse_)){
//				return AvailabilityStatus.notAvailable("卡券状态不可用");
//			}
//			if (cb.validate_type.equals(CouponConstants._coupon_validate_type_days_)) {
//				//固定领用天数
//				int a = Integer.parseInt(cb.validate_period);
//				Date date = new Date();//取时间
//				Calendar calendar  =   Calendar.getInstance();
//				calendar.setTime(date); //需要将date数据转移到Calender对象中操作
//				calendar.add(calendar.DATE, a);//把日期往后增加n天.正数往后推,负数往前移动
//				ccd.get_time=calendar.getTime();   //这个时间就是日期往后推一天的结果
//				DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//				int big = ccd.get_time.compareTo(date);
//				if (big<0) {
//					return AvailabilityStatus.notAvailable("超过卡券使用时间");
//				}
//			}
//		}
//
...
//			String[] s = goodsCouponRule.time_value.split("/");
//			boolean flag = true;
//			for(int i = 0;i<s.length();i++){
//				String[] ss = s[i].split("|");
//				String[] ss1 = s[0].split(",");
//				ss1 = DateUtil.compareM_to_X(ss1);
//				def flag1 = 0;
//				for(int j = 0;j<ss1.length();j++){
//					if(ss1[j].equals(Calendar.getInstance().get(Calendar.DAY_OF_WEEK))){
//						flag1++;
//						break;
//					}
//				}
//				String[] ss2 = s[1].split("-");
//				for(int z = 0;z<ss2.length();z++){
//					SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
//					if(DateUtil.compareH_to_L(ss2[0])<DateUtil.compareH_to_L(sdf.format(new Date()))&&DateUtil.compareH_to_L(sdf.format(new Date()))<DateUtil.compareH_to_L(ss2[0])){
//						flag1++;
//						break;
//					}
//				}
...
//			}
//			if(flag){
//				return AvailabilityStatus.notAvailable("没找到按周分时设置的时间");
//			}
//		}else if(goodsCouponRule.time_type.equals(CouponConstants._TIME_DAY_SHARING_)){
//			String[] t_value= goodsCouponRule.time_value.split("/");
//			//2018-09-01——2018-09-02|09:00-16:00
//			String big_time=null;
//			for (def y in t_value) {
//				String [] s=y.split("——");
//				String [] i=s[1].split("|");
//				String [] o =i[1].split("-");
//				String start_time=s[0]+o[0];
//				String end_time=i[0]+o[1];
//				Date date =new Date();
//				DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//				Date dateTime1 = dateFormat.parse(start_time);
//				Date dateTime2 = dateFormat.parse(end_time);
//				int big = dateTime1.compareTo(date);
//				int big_1=dateTime2.compareTo(date);
//				big_time +=big+","+big_1+"/";
//
//			}
//			String big_time1=big_time.substring(0,big_time.length()-1);
//			String [] big_time2= big_time1.split("/")
//			Boolean status=true;
//			for (def h in big_time2 ) {
//				String [] big_time3=h.split(",");
//				if (big_time3[0]<=0&&big_time3[1]>=0) {
//					status = false;
//					break ;
//				}
//			}
//			if (status==true) {
//				return AvailabilityStatus.notAvailable("没找到按天分时设置的时间");
//			}
//		}
//		//判断统一时间失效是否超期
//		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//		if(new Date().getTime()-df.parse(cb.validate_period).getTime()>0){
//			return AvailabilityStatus.notAvailable("已超过统一时间失效期限");
//		}
//		return true;
	}
	...
	public def consumeCouponPrice(List<CouponCode> couponCodes, BigInteger sid, def goods, def totalAmount,def param) {
		...
		try {
			...
			GoodsCouponRule goodsCouponRule = GoodsCouponRuleServer.INSTANSE.getRule_ByBathId(couponCodes.get(0).batch_id);
			...
			if (!StringUtil.isEmpty(goodsCouponRule.goods)) {
				...
				for (goc in item) {
					for(good in goods){
						if(goc.equals(good[0].toString()) && CouponConstants.goods_type_code.equals(good[1].toString()) ){
...
//			// 数量策略判断
			...
			for (def g : goodList) {
				availableTotalAmount = MoneyUtil.add(g[3].toString(), availableTotalAmount);
			}
			...
			if(CouponConstants._coupon_type_voucher_.equals(rule_type)){   //代金
				...
				if (exchange_goods >= goodsSum) {
					...
					for (int i = 0; i < exchange_goods; i++) {
						exchangeOriginalAmount = MoneyUtil.add(exchangeOriginalAmount, goodList[i][3].toString());
					}
	...
	private static def getByPriceDes(def g) {
		...
		for (def dto : g) {
			...
			for(int i = 0; i < list.size(); i++){
				def item = list.get(i);
				if(new BigDecimal(MoneyUtil.add(dto[3].toString(),"-" +item[3].toString())) >= 0){
					list.add(i,dto);
					flag = true;
					break;
				}
		...
		// 把商品拆成 单个商品
		for (def dto : list) {
			def num = new BigDecimal(dto[4].toString());
			dto[4] = "1";
			for (int i = 0; i < num; i++) {
				itemList.add(dto);
			}
...
//		String a = '周一,周二,周三,周四,周五,周六,周日;05:00;18:00';
//		def b = a.split("//|")
		print 10 / 2 * 3;
//		def p = '{"card_code":"53","g":[[8467,"100",null,20000.00,20.00],[8467,"100",null,10000.00,2.00]]}';
//		def s = new Date().getTime();
//		def deto  = JackSonBeanMapper.jsonToMap(p);
//		getByPriceDes(deto.g)
		//println JackSonBeanMapper.toJson();
//		def e = new Date().getTime();
//		println e-s 
//		3 : 2 补 10;
//		1: 0.6 不
//		10 20
