...
try {
	String coupon_code = JC.internal.param("cc");
	def pid = JC.internal.param("pid");
	Logger.info('{t_num= '+t_num+ ', coupon_code='+coupon_code + ',pid='+pid);
	if (pid == null || StringUtil.isEmpty(coupon_code)) {
		mcc.code = JsConstants.unknown;;
		mcc.msg = "参数不能为空";
		return mcc;
	}
	pid = pid.toString();
	List<CouponCode> list = CouponService.INSTANSE.get_coupon_by_code(coupon_code, new BigInteger(pid));
